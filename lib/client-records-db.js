...
export async function listClientRecords() {
  ...
  return rows.map((row) => ({
    "Client": row.client || "",
    "Website": row.website || "",
    "Homepage Title": row.homepage_title || "",
    "Homepage Meta Description": row.homepage_meta_description || "",
    "Homepage Checked At": row.homepage_checked_at ? row.homepage_checked_at.toISOString() : "",
    "Location": row.location || "",
    "Industry": row.industry || "",
    "Practice Type": row.practice_type || "",
    "Pod": row.pod || "",
