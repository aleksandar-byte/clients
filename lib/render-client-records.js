...
function renderRows(rows) {
  return rows.map((row, index) => {
    ...
    const practiceType = normalizeSpace(row["Practice Type"]);
    ...
    return `
      <tr class="row" data-status="${statusClass}" data-active-seo="${activeSeo}" data-pod="${escapeHtml(pod.toLowerCase())}" data-industry="${escapeHtml(industry.toLowerCase())}" data-practice-type="${escapeHtml(practiceType.toLowerCase())}" data-search="${escapeHtml(searchText)}" data-sort-name="${escapeHtml(clientName.toLowerCase())}" data-sort-date="${escapeHtml(startDate || launchDate)}">
        ...
        <td class="extra-col">${dash(practiceType)}</td>
...
function renderCards(rows) {
  return rows.map((row, index) => {
    ...
    const entryNo = String(index + 1).padStart(3, "0");
    return `
      <article class="card-item" data-status="${statusClass}" data-active-seo="${activeSeo}" data-pod="${escapeHtml(pod.toLowerCase())}" data-industry="${escapeHtml(industry.toLowerCase())}" data-practice-type="${escapeHtml(practiceType.toLowerCase())}" data-search="${escapeHtml(searchText)}" data-sort-name="${escapeHtml(clientName.toLowerCase())}" data-sort-date="${escapeHtml(startDate || launchDate)}">
        ...
        <div class="card-meta">${dash(gbpBusinessName)} <span>/</span> ${dash(location)} <span>/</span> ${dash(industry)} <span>/</span> ${dash(practiceType)} <span>/</span> ${dash(pod)}</div>
        <dl class="card-grid">
          <div class="extra-col"><dt>Practice type</dt><dd>${dash(practiceType)}</dd></div>
...
export function renderClientRecordsHtml(rows, options = {}) {
  ...
  const practiceTypes = [...new Set(rows.map((row) => normalizeSpace(row["Practice Type"])).filter(Boolean))]
    .sort((a, b) => a.localeCompare(b));
  const industryOptions = industries
    .map((industry) => `<option value="${escapeHtml(industry.toLowerCase())}">${escapeHtml(industry)}</option>`)
    .join("");
  const practiceTypeOptions = practiceTypes
    .map((practiceType) => `<option value="${escapeHtml(practiceType.toLowerCase())}">${escapeHtml(practiceType)}</option>`)
    .join("");
