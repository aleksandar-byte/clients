(() => {
  ...
  if (nav && navLinks) {
    ...
    window.addEventListener('resize', () => {
      if (window.innerWidth > 1040) {
        closeMenu();
        nav.querySelectorAll('.review-menu').forEach((menu) => menu.removeAttribute('open'));
      }
    });
  }
  ...
  document.querySelectorAll('form[data-review-form]').forEach((form) => {
    form.addEventListener('submit', (event) => {
      ...
      let status = form.querySelector('.review-form-status');
      ...
      status.textContent = 'Review form only. No information was sent.';
    });
  });
