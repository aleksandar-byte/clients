...
<body><div class="app"><aside class="sidebar"><div><div class="brand">Roster<span>&amp;</span>Co.</div><div class="brand-sub">Client Records</div></div><div class="side-section"><h4>By Status</h4><div class="side-link" data-filter-type="status" data-filter-value=""><span>All clients</span><span class="count">{summary["total"]:03d}</span></div><div class="side-link active" data-filter-type="status" data-filter-value="active"><span>Active</span><span class="count">{summary["active"]:03d}</span></div><div class="side-link" data-filter-type="inactive" data-filter-value="true"><span>Paused / churned</span><span class="count">{summary["inactive"]:03d}</span></div><div class="side-link" data-filter-type="status" data-filter-value="review"><span>Needs review</span><span class="count">{summary["review"]:03d}</span></div></div><div class="side-section"><h4>By Pod</h4><div class="side-link" data-filter-type="pod" data-filter-value="white"><span><span class="pod-tag pod-white-bg"></span>White Pod</span><span class="count">{summary["white"]:03d}</span></div><div class="side-link" data-filter-type="pod" data-filter-value="orange"><span><span class="pod-tag pod-orange-bg"></span>Orange Pod</span><span class="count">{summary["orange"]:03d}</span></div></div><div class="side-foot"><div><span class="meta">Source</span></div><div>Airtable / Clients</div><br><div><span class="meta">Rendered</span></div><div>{html.escape(issued)}</div></div></aside><main class="main"><div class="topbar"><div class="search"><input id="search" type="search" placeholder="Search clients, locations, geo, notes..." autocomplete="off"><span class="kbd">/</span></div><div class="topbar-spacer"></div><div class="topbar-meta">Roster / <strong>{summary["total"]} entries</strong></div><div class="view-toggle"><button id="view-table" class="active" type="button">Table</button><button id="view-cards" type="button">Cards</button></div></div><header class="masthead"><div class="spec-strip"><span>Issued <strong>{html.escape(issued)}</strong></span><span>/</span><span>SEO & PPC roster</span><span>/</span><span>{summary["total"]} records</span></div><div class="title-grid"><h1>The Roster, <em>annotated</em>.</h1><div class="title-deck">A cleaner field guide to every client: sorted, searchable, linked, and easier to review.</div></div><p class="lede">{source_description} Update upstream first; this view rebuilds from the records pipeline. API identifiers are hidden unless you turn them on.</p><div class="stats"><div class="stat"><div class="stat-label">Total Roster</div><div class="stat-value">{summary["total"]}</div><div class="stat-trend">All tracked clients</div></div><div class="stat"><div class="stat-label">Active</div><div class="stat-value">{summary["active"]}</div><div class="stat-trend">{summary["active_pct"]}% of roster</div><div class="stat-bar"><span style="width:{summary["active_pct"]}%;background:var(--forest)"></span></div></div><div class="stat"><div class="stat-label">Paused / Churned</div><div class="stat-value">{summary["inactive"]}</div><div class="stat-trend">{pct(summary["inactive"], summary["total"])}% inactive</div><div class="stat-bar"><span style="width:{pct(summary["inactive"], summary["total"])}%;background:var(--crimson)"></span></div></div><div class="stat"><div class="stat-label">White Pod</div><div class="stat-value">{summary["white"]}</div><div class="stat-trend">{summary["white_pct"]}% share</div><div class="stat-bar"><span style="width:{summary["white_pct"]}%;background:var(--blue)"></span></div></div><div class="stat"><div class="stat-label">Orange Pod</div><div class="stat-value">{summary["orange"]}</div><div class="stat-trend">{summary["orange_pct"]}% share</div><div class="stat-bar"><span style="width:{summary["orange_pct"]}%;background:var(--accent)"></span></div></div></div></header><section class="workspace"><div class="section-head"><h2>The <em>full</em> ledger</h2><div class="rule"></div><div class="meta-tag">{summary["total"]} entries / sorted A-Z</div></div><div class="toolbar"><span class="toolbar-label">Filter -></span><select id="industry-filter"><option value="">All industries</option>
{industry_options}
        </select><select id="pod-filter"><option value="">All pods</option><option value="white">White pod</option><option value="orange">Orange pod</option></select><select id="status-filter"><option value="">All statuses</option><option value="active" selected>Active</option><option value="inactive">Paused / churned</option><option value="review">Needs review</option></select><label class="toggle-field"><input id="show-api-keys" type="checkbox"> Show API keys</label><label class="toggle-field"><input id="show-notes" type="checkbox"> Show notes</label><button class="chip" id="reset-btn" type="button" style="display:none;">Clear x</button><div class="results-count" id="results-count">Showing <strong>{summary["active"]}</strong> of {summary["total"]}</div></div><div class="table-shell" id="table-view"><div class="table-scroll"><table><thead><tr><th>Client</th><th>Location</th><th>Industry</th><th>Pod</th><th>Started</th><th>Services</th><th>Status</th><th class="api-key-col">Monday</th><th class="api-key-col">GA4</th><th class="api-key-col">GSC</th><th>Geo targets</th><th class="notes-col">Notes</th></tr></thead><tbody id="rows">
{''.join(row_html)}
      </tbody></table></div></div><div class="cards" id="card-view">
{''.join(card_html)}
      </div><div class="empty" id="empty-state"><h3>Nothing in the ledger.</h3><p>Clear filters or search again.</p></div></section></main></div><script>(function(){{const search=document.getElementById('search');const podFilter=document.getElementById('pod-filter');const statusFilter=document.getElementById('status-filter');const industryFilter=document.getElementById('industry-filter');const resetBtn=document.getElementById('reset-btn');const empty=document.getElementById('empty-state');const tableView=document.getElementById('table-view');const cardView=document.getElementById('card-view');const sideLinks=document.querySelectorAll('.side-link');const viewTable=document.getElementById('view-table');const viewCards=document.getElementById('view-cards');const showApiKeys=document.getElementById('show-api-keys');const showNotes=document.getElementById('show-notes');const total={summary["total"]};function applyFilters(){{const q=(search.value||'').trim().toLowerCase();const pod=podFilter.value;const status=statusFilter.value;const industry=industryFilter.value;const items=[...document.querySelectorAll('#rows .row'),...document.querySelectorAll('#card-view .card-item')];let visible=0;items.forEach(el=>{{const matchesStatus=!status||el.dataset.status===status||(status==='inactive'&&['paused','lost'].includes(el.dataset.status));const show=(!q||el.dataset.search.includes(q))&&(!pod||el.dataset.pod===pod)&&matchesStatus&&(!industry||el.dataset.industry===industry);el.classList.toggle('hidden',!show);if(show&&el.classList.contains('row'))visible++;}});document.getElementById('results-count').innerHTML='Showing <strong>'+visible+'</strong> of '+total;resetBtn.style.display=(q||pod||status||industry)?'inline-flex':'none';empty.classList.toggle('show',visible===0);tableView.style.display=visible===0?'none':(viewTable.classList.contains('active')?'':'none');cardView.style.display=visible===0?'none':(viewCards.classList.contains('active')?'grid':'none');sideLinks.forEach(link=>{{const type=link.dataset.filterType;const value=link.dataset.filterValue;link.classList.toggle('active',(type==='status'&&value===status)||(type==='inactive'&&status==='inactive')||(type==='pod'&&value===pod)||(!value&&!status&&!pod&&type==='status'));}});}}search.addEventListener('input',applyFilters);podFilter.addEventListener('change',applyFilters);statusFilter.addEventListener('change',applyFilters);industryFilter.addEventListener('change',applyFilters);resetBtn.addEventListener('click',()=>{{search.value='';podFilter.value='';statusFilter.value='';industryFilter.value='';applyFilters();}});sideLinks.forEach(link=>link.addEventListener('click',()=>{{const type=link.dataset.filterType;const value=link.dataset.filterValue;if(type==='status'){{statusFilter.value=value;if(!value){{podFilter.value='';industryFilter.value='';search.value='';}}}}if(type==='inactive')statusFilter.value='inactive';if(type==='pod')podFilter.value=(podFilter.value===value)?'':value;applyFilters();}}));viewTable.addEventListener('click',()=>{{viewTable.classList.add('active');viewCards.classList.remove('active');tableView.style.display='';cardView.style.display='none';}});viewCards.addEventListener('click',()=>{{viewCards.classList.add('active');viewTable.classList.remove('active');tableView.style.display='none';cardView.style.display='grid';}});showApiKeys.addEventListener('change',()=>document.body.classList.toggle('show-api-keys',showApiKeys.checked));showNotes.addEventListener('change',()=>document.body.classList.toggle('show-notes',showNotes.checked));document.addEventListener('keydown',e=>{{if(e.key==='/'&&document.activeElement!==search){{e.preventDefault();search.focus();}}if(e.key==='Escape'&&document.activeElement===search){{search.value='';applyFilters();search.blur();}}}});applyFilters();}})();</script></body></html>"""
