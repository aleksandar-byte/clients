...
FIELD_ORDER = [
    ...
    "Geo Targets Done",
    "Geo Targets Planned",
    "GSC Google Search Console Secondary Locations",
    "WhatConverts Account ID",
    "WhatConverts Account Name",
    "WhatConverts Profile ID",
    "WhatConverts Profile Name",
...
def render_html(rows: list[dict[str, str]], base_id: str, table_name: str) -> str:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        gsc_secondary_locations = normalize_space(row.get("GSC Google Search Console Secondary Locations"))
        ...
        whatconverts_account_id = normalize_space(row.get("WhatConverts Account ID"))
        whatconverts_account_name = normalize_space(row.get("WhatConverts Account Name"))
        whatconverts_profile_id = normalize_space(row.get("WhatConverts Profile ID"))
        whatconverts_profile_name = normalize_space(row.get("WhatConverts Profile Name"))
        gbp_business_name = normalize_space(row.get("GBP Business Name"))
        gbp_link = normalize_space(row.get("GBP Link"))
        web_build_sheet = normalize_space(row.get("Web Build Sheet"))
        site_sitemap_xml = normalize_space(row.get("Site Sitemap XML"))
        ...
        data_sort_name = html.escape(client_name.lower(), quote=True)
        data_sort_date = html.escape(start_date or launch_date, quote=True)
        entry_no = f"{index:03d}"
        row_html.append(
            f"""
              <tr class="row" data-status="{status_class}" data-active-seo="{active_seo}" data-pod="{data_pod}" data-industry="{data_industry}" data-search="{data_search}" data-sort-name="{data_sort_name}" data-sort-date="{data_sort_date}">
                ...
                <td class="api-key-col wide">{dash(gsc_secondary_locations)}</td>
                <td class="api-key-col mono">{dash(whatconverts_account_id)}</td>
                <td class="api-key-col">{dash(whatconverts_account_name)}</td>
                <td class="api-key-col mono">{dash(whatconverts_profile_id)}</td>
                <td class="api-key-col">{dash(whatconverts_profile_name)}</td>
                <td class="extra-col wide">{dash(web_build_sheet)}</td>
                <td class="extra-col wide">{dash(site_sitemap_xml)}</td>
        ...
        card_html.append(
            f"""
              <article class="card-item" data-status="{status_class}" data-active-seo="{active_seo}" data-pod="{data_pod}" data-industry="{data_industry}" data-search="{data_search}" data-sort-name="{data_sort_name}" data-sort-date="{data_sort_date}">
                ...
                <div class="card-meta">{dash(gbp_business_name)} <span>/</span> {dash(location)} <span>/</span> {dash(industry)} <span>/</span> {dash(pod_value)}</div>
                <dl class="card-grid">
                  <div class="extra-col"><dt>Started</dt><dd class="mono">{dash(start_date)}</dd></div>
                  <div class="extra-col"><dt>Launched</dt><dd class="mono">{dash(launch_date)}</dd></div>
                  <div class="extra-col"><dt>Services</dt><dd>{dash(services)}</dd></div>
                  ...
                  <div class="api-key-col span-2"><dt>GSC secondary locations</dt><dd class="truncate">{dash(gsc_secondary_locations)}</dd></div>
                  <div class="api-key-col"><dt>WC account ID</dt><dd class="mono">{dash(whatconverts_account_id)}</dd></div>
                  <div class="api-key-col"><dt>WC profile ID</dt><dd class="mono">{dash(whatconverts_profile_id)}</dd></div>
                  <div class="api-key-col span-2"><dt>WC account</dt><dd class="truncate">{dash(whatconverts_account_name)}</dd></div>
                  <div class="api-key-col span-2"><dt>WC profile</dt><dd class="truncate">{dash(whatconverts_profile_name)}</dd></div>
                  <div class="extra-col span-2"><dt>Web build sheet</dt><dd class="truncate">{dash(web_build_sheet)}</dd></div>
                  <div class="extra-col span-2"><dt>Site sitemap XML</dt><dd class="truncate">{dash(site_sitemap_xml)}</dd></div>
